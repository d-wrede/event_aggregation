...
for message in messages:
    ...
    for i, date_pattern in enumerate(date_patterns):
        dates = re.findall(date_pattern, message['message'], flags=re.IGNORECASE)
        ddates = re.findall(ddate_patterns[i], message['message'], flags=re.IGNORECASE)
        ...
        for ddate in ddates:
            ...
            # print("found ddate! : ", start_date, " - ", end_date)
            # dateset.add((day1, month, year))
            # dateset.add((day2, month, year))
    ...
    print("clockset: ", clockset)

    if dateset and clockset:
        if start_date is None or end_date is None:
            # Convert date strings to datetime.date objects)
            dates = [datetime.date(2000 + int(year), int(month), int(day)) for year, month, day in dateset]
        ...
        times = [datetime.time(hour=int(hour), minute=int(minute)) for hour, minute in clockset]

        # Find min and max times
        start_time = min(times)
        end_time = max(times)
        print(f"Start: on the {start_date} at {start_time} o'clock\nEnd: on the {end_date} at {end_time} o'clock.")
    else:
